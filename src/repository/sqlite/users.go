...
func (repo *UsersRepository) GetById(id int64) (*entities.User, error) {
	...
	defer cancel()
	query := fmt.Sprintf("SELECT %[1]s.id, %[1]s.tg_id, %[1]s.group_id, %[1]s.full_name, %[2]s.role_name FROM %[1]s INNER JOIN %[2]s ON %[1]s.id = %[2]s.user_id WHERE %[1]s.id = $1", USERS_TABLE, ROLES_TABLE)
...
func (repo *UsersRepository) Update(user *entities.User) error {
	...
	query := fmt.Sprintf("UPDATE %s SET tg_id=$1, group_id=$2, full_name=$3", USERS_TABLE)
	_, err = tx.ExecContext(ctx, query, user.TgId, user.GroupId, user.FullName)
	if err != nil {
		return err
	}
...
func (repo *UsersRepository) Delete(id int64) error {
	ctx, cancel := context.WithTimeout(context.Background(), QUERY_TIMEOUT)
	defer cancel()
	query := fmt.Sprintf("DELETE FROM %s WHERE user_id=$1", USERS_TABLE)
